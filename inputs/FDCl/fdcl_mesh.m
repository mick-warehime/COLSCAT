function mesh = fdcl_mesh(Av,n,vinput,ploton)

% set masses
mh = 1836.15264;                    % mass of hydrogen in au
m  = mh*[19.00 2*1.008 35.453]/1.008; % mass of [F,H,Cl] in amu
vin = 0.06;                         % countour of inner boundary
vout = 0.3;                         % countour of outer boundary
rmax = 11;                          % cutoff value for reactant
pmax = 11;                          % cutoff value for product
...
mesh = meshgen(m,vin,vout,rmax,pmax,xj,yj,vj,npar,n,ploton,'fdcl_');
