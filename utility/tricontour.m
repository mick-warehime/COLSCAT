function [cout,hout] = tricontour(p,t,Hn,N,zLoc)
...
for v = 1:num       % Loop over contouring levels
    ...
    if nargout>0
        ...
        for k = 1:ncc
            ...
            if (ce==0)||(ce==start)||(flag(ce))     
                ...
                h   = [h; patch('Xdata',[x,NaN],'Ydata',[y,NaN],'Zdata',z, ...
                                'Cdata',zLoc*ones(size(z)),'facecolor','none','edgecolor','flat')]; hold on      
        ...
        patch('Xdata',[penew(c1,1),penew(c2,1)]', ...
              ...
              'Zdata',zLoc*ones(size(z)),'Cdata',z,'facecolor','none','edgecolor','flat'); 
